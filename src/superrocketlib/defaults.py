...
STANDARD_COMPETITION_ROCKET_RANGES = RocketRanges(
    radius=Range.discrete(0.145, 0.160, 0.005),
    length=Range.continuous(2.8, 4.0),
    mass=Range.continuous(15.0, 30.0),
    center_of_mass_without_motor=Range.continuous(1.2, 2.2),
    I_11_without_motor=Range.continuous(0.5, 2.0),
    I_22_without_motor=Range.continuous(0.5, 2.0),
    I_33_without_motor=Range.continuous(0.1, 0.5),
    I_12_without_motor=Range.continuous(-0.1, 0.1),
    I_13_without_motor=Range.continuous(-0.1, 0.1),
    I_23_without_motor=Range.continuous(-0.1, 0.1),
    wall_thickness=Range.continuous(0.003, 0.006),
    motor_position=Range.continuous(0.3, 0.8),
)

STANDARD_COMPETITION_MOTOR_RANGES = MotorRanges(
    motor_diameter=Range.continuous(0.12, 0.145),
    motor_length=Range.continuous(1.0, 1.8),
    nozzle_radius=Range.continuous(0.02, 0.04),
    throat_radius=Range.continuous(0.01, 0.025),
    dry_mass=Range.continuous(6.0, 12.0),
    dry_I_11=Range.continuous(0.05, 0.2),
    dry_I_22=Range.continuous(0.05, 0.2),
    dry_I_33=Range.continuous(0.02, 0.08),
    dry_I_12=Range.continuous(-0.02, 0.02),
    dry_I_13=Range.continuous(-0.02, 0.02),
    dry_I_23=Range.continuous(-0.02, 0.02),
    grain_number=IntRange(3, 7),
    grain_density=Range.continuous(1600, 1850),
    grain_outer_radius=Range.continuous(0.04, 0.065),
    grain_initial_inner_radius=Range.continuous(0.015, 0.035),
    grain_initial_height=Range.continuous(0.2, 0.5),
    grain_separation=Range.continuous(0.01, 0.03),
    grains_center_of_mass_position=Range.continuous(0.2, 0.7),
    center_of_dry_mass_position=Range.continuous(0.2, 0.7),
    burn_time=Range.continuous(2.0, 6.0),
    average_thrust=Range.continuous(4000, 14000),
...
STANDARD_COMPETITION_FINS_RANGES = TrapezoidalFinsRanges(
    n=IntRange(3, 4),
    root_chord=Range.continuous(0.25, 0.5),
    tip_chord=Range.continuous(0.1, 0.25),
    span=Range.continuous(0.12, 0.3),
    sweep_length=Range.continuous(0.05, 0.2),
    cant_angle=Range.continuous(0.0, 2.0),
)
...
STANDARD_COMPETITION_ENVIRONMENT_RANGES = EnvironmentRanges(
    ...
    longitude=Range.continuous(-60.0, -40.0),
    elevation=Range.continuous(0.0, 200.0),
    max_expected_height=Range.continuous(2000.0, 80000.0),
)
